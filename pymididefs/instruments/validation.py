...
# Control names, voice names and file stems all share the grammar the project
# definition files already use, so a definition composes with them unchanged.
...
class _Reader:
	...
	def model (self, value: object) -> pymididefs.instruments.definition.Model:
		...
		return pymididefs.instruments.definition.Model(
	...
	def midi (self, value: object) -> pymididefs.instruments.definition.Midi:
		...
		if value == "none":
			return pymididefs.instruments.definition.Midi(stated_none = True)
		...
		if section.get("program_change") is not None and section["program_change"] != "none":
			...
			program_change = pymididefs.instruments.definition.ProgramChange(
		...
		return pymididefs.instruments.definition.Midi(
	...
	def voice (self, value: object) -> pymididefs.instruments.definition.Voice:
		...
		return pymididefs.instruments.definition.Voice(
	...
	def velocity (self, value: object) -> pymididefs.instruments.definition.Velocity | None:
		...
		return pymididefs.instruments.definition.Velocity(
	...
	def pitch_bend (self, value: object) -> pymididefs.instruments.definition.PitchBend | None:
		...
		return pymididefs.instruments.definition.PitchBend(
	...
	def controls (self, value: object) -> dict[str, pymididefs.instruments.definition.Control]:
	...
	def control (self, name: str, value: object) -> pymididefs.instruments.definition.Control:
		...
		kinds = pymididefs.instruments.definition.KINDS

		if kind is not None and kind not in kinds:
			self.refuse(f"{where}.kind", f"{kind!r} is not one of {sorted(kinds)}")

		control = pymididefs.instruments.definition.Control(
			name  = name,
...
def check_stem (stem: str, source: str) -> None:
	...
	"""Refuse a file whose name is not addressable.

	The stem is ``manufacturer_model``, and it is how a composition asks for the
	definition, so it has to obey the same grammar as everything else here.
	"""

	if not NAME.fullmatch(stem):
		raise DefinitionError(
			f"{source}: file name: {stem!r} is not a name — "
			f"use lower case, digits and underscores, as in moog_matriarch"
		)
