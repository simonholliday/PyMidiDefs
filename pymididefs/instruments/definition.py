...
**specification** facts in the rest of this package, which are true of all MIDI
everywhere, and from **rig** facts, which belong to one studio and never appear
here.
...
# Every kind a definition may name. Built here, where the names are local: a
# module-level lookup through `pymididefs.instruments.…` would run while this
# package is still importing, and the attribute is not bound until it finishes.
